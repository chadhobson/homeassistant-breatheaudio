"""Config flow for BreatheAudio 6-Zone Amplifier integration."""
import logging

from .pybreatheaudio import get_async_breatheaudio
...
async def validate_input(hass: core.HomeAssistant, data):
    ...
    """
    try:
        await get_async_breatheaudio(data[CONF_PORT], hass.loop)
    except SerialException:
        _LOGGER.error("Error connecting to BreatheAudio controller")
        raise CannotConnect
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for BreatheAudio 6-Zone Amplifier."""
...
class BreatheAudioOptionsFlowHandler(config_entries.OptionsFlow):
    """Handle a BreatheAudio options flow."""
